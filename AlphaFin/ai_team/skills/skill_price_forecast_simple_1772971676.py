"""
自动生成的技能: price_forecast_simple
描述: 简化版个股价格区间预测技能，基于多因子评分模型
创建者: quant
"""
...
def calculate_technical_score(kline_data):
    ...
    ma5 = sum([d['close'] for d in kline_data[:5]]) / min(5, len(kline_data))
...
def calculate_price_forecast(current_price, composite_score, market_heat):
    ...
    return {
        ...
        'risk_reward_ratio': round((upside_factor - 1) / (1 - downside_factor), 2) if downside_factor < 1 else 999
    }
